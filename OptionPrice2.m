function V= OptionPrice2(x,z,T_1,T_2,nt, L, psi, r, q, V_initial, y_0)
% diagnoal matrix
...
% can make sure no arbitrage
    f(1)=exp(x(1))*exp(-r*(T_1+i*dt))*dx+1/2*exp(x(1))*exp(-r*(T_1+i*dt))*dx^2+1/6*exp(x(1))*exp(-r*(T_1+i*dt))*dx^3+ ...
        1/24*exp(x(1))*exp(-r*(T_1+i*dt))*dx^4+1/120*exp(x(1))*exp(-r*(T_1+i*dt))*dx^5;
    f(end)=0;
    V=M\f;
    V(V<0)=0;
end
